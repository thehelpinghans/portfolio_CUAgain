	...
	$.ajax({
		...
		success:function(responsedHtml){
			...
			if(formData.order==1){
				...
				movieInfoOfNaverApi(formData.movieName);
			}
...
//네이버api를 이용해서 내가필요한 영화정보를 HTML 마지막 응답메세지에  내부(.select-list)위치에 출력
function movieInfoOfNaverApi(movieName){
	$.ajax({
		url:"/movie-bot/search/naverApi",
...
function movieNameClicked(el){
	...
	//선택한 영화제목이 챗팅창에 출력
	//var tagStyle=inputTagString(movieName);
	//*
